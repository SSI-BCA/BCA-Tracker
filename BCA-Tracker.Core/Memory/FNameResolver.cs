...
namespace BCATracker.Core
{
    ...
    public class FNameResolver
    {
        ...
        // Retry policy: when initialisation fails (typical on early attach
        // before the game finished bootstrapping), back off for a few seconds
        // and try again on the next Resolve() call. Without this, a missed
        // first-shot meant resolver was permanently dead until the user
        // restarted the tracker.
        DateTime _nextRetryUtc = DateTime.MinValue;
        int      _retryAttempts = 0;
        const int  MaxRetryAttempts = 12;     // ~ at 5 s cooldown that's 1 min of trying
        ...
        public FNameResolver(MemoryReader mem)
        {
            ...
            _logPath = Path.Combine(
                ...
                "BCA-Tracker", "fname_resolver.log");
        }
        ...
        public void EnsureInitialized()
        {
            ...
            if (_retryAttempts >= MaxRetryAttempts) return;
            TryInitialize();
        }
        ...
        public string Resolve(int comparisonIndex)
        {
            ...
            if (!_initialized)
            {
                if (DateTime.UtcNow < _nextRetryUtc) return null;
                if (_retryAttempts >= MaxRetryAttempts) return null;
                TryInitialize();
            }
        ...
        void TryInitialize()
        {
            try
            {
                ...
                if (moduleBase == 0)
                {
                    Log($"Init attempt {_retryAttempts + 1}: moduleBase=0");
                    ScheduleRetry();
                    return;
                }
                ...
                Log($"Init attempt {_retryAttempts + 1}: all strategies exhausted");
                ScheduleRetry();
            }
            catch (Exception ex)
            {
        ...
        void ScheduleRetry()
        {
            ...
            if (_retryAttempts >= MaxRetryAttempts)
                Log($"Giving up after {_retryAttempts} attempts. Restart the tracker once the game has fully loaded.");
        }
